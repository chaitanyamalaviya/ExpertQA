...
from transformers.trainer_utils import EvalLoopOutput, EvalPrediction, PredictionOutput, get_last_checkpoint, speed_metrics
...
def wandb_hp_space(trial):
    return {
        "method": "random",
        "metric": {"name": "best_f1", "goal": "maximize"},
        "parameters": {
            "learning_rate": {'values': [3e-5, 1e-4]},
            "per_device_train_batch_size": {"values":  [1, 2, 8, 16]},
        },
    }
...
class QuestionAnsweringSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def evaluate(
        ...
        **gen_kwargs,
        ...
        preds = np.where(output.predictions != -100, output.predictions, self.tokenizer.pad_token_id)
        text_outputs = self.tokenizer.batch_decode(preds, skip_special_tokens=True, clean_up_tokenization_spaces=True)
        # text_outputs = self.tokenizer.batch_decode(output.predictions, skip_special_tokens=True)
        val_data = []
        with open(val_filename) as fp:
            val_data = [json.loads(line) for line in fp.readlines()]
        for i, out in enumerate(text_outputs):
            val_data[i]["output"] = [out]
            val_data[i]["context_provided"] = self.tokenizer.decode(eval_dataset[i]['input_ids'])
            val_data[i]["context_raw"] = output.predictions.tolist()

        with open(f"{self.args.output_dir}/test_eval_outputs.json", "w") as fp:
    ...
    def predict(
        ...
        for i, out in enumerate(text_outputs):
            val_data[i]["output"] = [out]
            val_data[i]["context_provided"] = self.tokenizer.decode(predict_dataset[i]['input_ids'])
...
class ModelArguments:
    ...
    cache_dir: Optional[str] = field(
        default="/nlp/data/huggingface_cache/",
...
question_answering_column_name_mapping = {
    ...
    "nq": ("question", "context", "answer"),
    "drop": ("question", "context", "answer")
}
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.validation_file is not None:
            ...
            val_filename = data_args.validation_file
    ...
    def preprocess_validation_function(examples):
        ...
        for i in range(len(model_inputs["input_ids"])):
            model_inputs["example_id"].append(examples["example_id"][i])
            labels_out.append(labels["input_ids"][i])
            # One example can give several spans, this is the index of the example containing this span of text.
            # sample_index = sample_mapping[i]
            # model_inputs["example_id"].append(examples["example_id"])
    ...
    def post_processing_function(
        ...
        for example_index, example in enumerate(examples):
            ...
            predictions[example["example_id"]] = decoded_preds[feature_index]
        ...
        # references = [{"id": ex["example_id"], "answers": ex[answer_column]} for ex in examples]
        references = [{"id": str(ex["example_id"]), "answers": {"text": [ex[answer_column]], "answer_start": [0]}} for ex in examples]
