# copied from https://github.com/shmsw25/FActScore/blob/main/factscore/atomic_facts.py
...
class AtomicFactGenerator(object):
    def __init__(self, demon_dir, gpt3_cache_file=None):
        self.nlp = spacy.load("en_core_web_sm")
        self.is_bio = True
        self.demon_path = os.path.join(demon_dir, "demons.json" if self.is_bio else "demons_complex.json")

        self.openai_lm = OpenAIModel("InstructGPT", cache_file=gpt3_cache_file)
...
def main():
    ...
    generator = AtomicFactGenerator("modeling/fact_score/demos", gpt3_cache_file='.cache/factscore')

    ## load dataset
    examples = example_utils.read_examples(args.input_file)
    for example in examples:
        answer = example.answers[list(example.answers.keys())[0]]
        for claim in answer['claims']:
            claim_string = re.sub(r'\[\d+\]', '', claim['claim_string'])
            if not claim_string.strip():
                claim['atomic_claims'] = []
            atomic_facts = generator.run(claim_string)
            claim['atomic_claims'] = atomic_facts[0][0][1]
